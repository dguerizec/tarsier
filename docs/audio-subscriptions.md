...
Find source IDs and their `enabled` status through `GET /api/v1/audio/sources`.
The source must already be enabled for capture in Tarsier. Subscribing does not
open another device reader, enable a disabled input or change camera/audio settings.
The virtual output itself cannot be used as the subscription source.
...
```json
{
  ...
  "subscription": {"type":"subscribe", "source":"the-enabled-source-id", "start":{"event":"gesture.phone_near_mouth.started"}, "end":{"event":"gesture.phone_near_mouth.ended"}, "pre_roll_ms":300, "max_duration_ms":60000},
...
Raw capture is independent of the virtual microphone's mute, automatic gain and
voice conversion. It therefore continues serving an authorized subscription while
the virtual microphone is muted. This is intended for the independent agent use
case; future conference/agent priority controls are not implemented here.
...
use daemon acquisition timestamps and sample-aligned clipping; they are not
...
`code` and the connection closes. Shutdown, disabled capture, revocation, stalled
...
The default input is Tarsier's selected microphone; use `--source SOURCE_ID` to
choose another enabled capture source.
